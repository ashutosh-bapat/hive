...
public class CreateTableDesc extends DDLDesc implements Serializable {
  ...
  public Table toTable(HiveConf conf) throws HiveException {
    ...
    if (colStats != null) {
      ...
      // Don't bother touching table stats if it's being replicated.
      // TODO: we may want to do this only when we are replicating data as well.
      if (!this.isCTAS && (tbl.getPath() == null || (tbl.isEmpty() && !isExternal()))) {
        if (!tbl.isPartitioned() && conf.getBoolVar(HiveConf.ConfVars.HIVESTATSAUTOGATHER)) {
          LOG.debug("Calling setStatsStateForCreateTable(cols, true) on " + tbl.getDbName() + "." + tbl.getTableName());
          StatsSetupConst.setStatsStateForCreateTable(tbl.getTTable().getParameters(),
                  MetaStoreUtils.getColumnNames(tbl.getCols()), StatsSetupConst.TRUE);
        }
      } else {
        LOG.debug("Calling setStatsStateForCreateTable(null, false) on " + tbl.getDbName() + "." + tbl.getTableName());
